...
```text
DuoLayerAC/
├── zk-Guard-chaincode/          # Fabric 链码与构建脚本
└── zk-Guard-client/
...
保留 `zk-Guard-*` 子目录名是为了与原始 Fabric test-network 实验路径兼容，并
明确标识协同框架复用的控制层组件。
...
`zk-Guard-client/` 内执行脚本。

## 功能验证

```bash
cd fabric-samples/test-network/zk-Guard-client
